...
| `ctx.oauth` | `core` | [`llm-oauth`](../packages/llm/llm-oauth) | - | [`llm-pi-ai`](../packages/llm/llm-pi-ai) | - | Owns device-flow login, durable credentials, and background token refresh; adapters consume refreshed access tokens through the credentials seam. |
